...
// Verificar si la IP de destino existe
if ((new CheckDevice())->ping($ip_addr)) {
    echo "La IP de destino existe. Continuando con el proceso...\n";
    ...
    $log_entry .= "IP de Destino: $ip_addr\n";
    ...
    if (in_array($remote_ip, $allowed_ips)) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $log_entry .= "  IP del Equipo Destino: $ip\n";
            ...
            if (!empty($admin_user) && !empty($admin_pass) && !empty($ip) && !empty($target_user) && !empty($user_pass)) {
                ...
                if (trim($output) === 'true') {
                    ...
                    $log_entry .= "El usuario especificado no existe en el equipo destino.\n";
                    echo "El usuario especificado no existe en el equipo destino.";
                } else {
    ...
    echo "La IP de destino no existe. No se han efectuado cambios.\n";
}
